...
steps/make_plp.sh --plp-config conf/plpTelAntho.conf --nj ${nji} --cmd \"$train_cmd\" ${name} ${name}/log $mfccdir || exit 1
...
steps/compute_cmvn_stats.sh   $name   $name/log   $mfccdir
...
./bin/decode_innetQuad_phone.sh --nnet $model/final.nnet --srcdir $model/ --phi $phi --rescore $lm --num_threads $threads --use_gpu $gpu  --skip_scoring true --nj $nj --cmd "$decode_cmd" --cmd2 "$rescore_cmd"  $graph/ ${name} ${name}/results || exit 1
date
duration=$SECONDS
