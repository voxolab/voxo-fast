...
if (@ARGV < 1 || @ARGV > 2) {
  print STDERR "Usage: convert_ctm.pl <segments-file>  [<utterance-ctm>] > real-ctm\n";
  exit(1);
}
...
while(<S>) {
  ...
  $end{$utt} = $end_time;
  $reco2file{$recording_id} = $recording_id;
  $reco2channel{$recording_id} =1;
}
...
#  $channel = $reco2channel{$reco};
  ...
  if (defined $conf) {
    $line = "$file $channel $wbegin_r $wlen $w $conf $utt\n"; 
  } else {
    $line = "$file $channel $wbegin_r $wlen $w 1.0 $utt\n"; 
  }
